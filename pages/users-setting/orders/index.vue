<template>
  <div class="flex-1 divide-y divide-accented w-full shadow-2xl bg-gray-50 dark:bg-gray-800 border border-gray-300 dark:border-gray-800 p-4 rounded-xl">
    <HeaderPageHead title="Orders" />
    
    <div class="flex max-md:flex-col items-center gap-2 px-4 py-3.5 overflow-x-auto">
      <!-- Filter by User -->
      <USelectMenu
        placeholder="Filter by users"
        :items="[{ label: 'All', value: 'All' }, ...(users?.map(user => ({
          label: user.fullname,
          value: user.username,
        })) || [])]"
        class="w-48 max-md:w-full"
        @update:model-value="table?.tableApi?.getColumn('user')?.setFilterValue($event)"
      />
...
import type { User } from '~/types/users';
  ...
  definePageMeta({
    ...
    requiredPermission: 'list-orders',
  })
  ...
  const columns: TableColumn<Order>[] = [{
    ...
    cell: ({ row }) => h('div', { class: 'w-full flex items-center justify-between' }, [
      ...
      h(
        ...
        {
          to: `/users-setting/orders/${row.getValue('id')}`,
    ...
    cell: ({ row }) => {
      const user = row.getValue('user') as { fullname?: string } | null
      return h('div', { class: 'w-full flex items-center justify-between' }, [
        h('p', { class: 'text-sm font-medium hidden max-md:block' }, "User:"),
        h('p', { class: '' }, user?.fullname || '-')
      ])
    },
    filterFn: (row, columnId, filterValue) => {
      if (!filterValue.value || filterValue.value === 'All') return true
      const user = row.getValue(columnId) as User | null | undefined
      return user?.username === filterValue.value
    },
  }, {
    accessorKey: 'status',
    header: ({ column }) => {
